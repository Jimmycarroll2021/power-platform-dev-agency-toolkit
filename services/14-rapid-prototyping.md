...
- **Premium connectors** — if the prototype uses any premium connector (e.g. SQL Server, HTTP, custom connectors, most third-party connectors), production users will need a **Power Apps Premium (per-user) or Per-App** license. Standard Microsoft 365 seeded licenses do **not** cover premium connectors. (Needs verification against current Microsoft docs.)
- **Power Apps licensing** — confirm whether the production app needs Premium per-user vs Per-App, and how many seats. Exact per-user / per-app price points change — (Needs verification against current Microsoft docs).
- **AI Builder credits** — any AI Builder model (document processing, prediction, GPT prompts) consumes AI Builder credits, sold in capacity packs. Prototype usage is cheap; production volume is not. Estimate monthly document/transaction volume and flag the credit pack requirement. Exact credit pricing and pack sizes — (Needs verification against current Microsoft docs).
- **Copilot Studio messages** — Copilot Studio agents are metered by **messages** (capacity packs or pay-as-you-go). A prototype agent burns almost nothing; a deployed agent fielding real traffic needs a sized message-capacity estimate. Exact message rates and pack sizing — (Needs verification against current Microsoft docs).
- **Dataverse capacity** — Dataverse database, file, and log storage is metered. A prototype's tables are trivial, but flag expected production row counts and attachment volumes so storage capacity is in the production estimate. Default capacity entitlements depend on the tenant's license mix — (Needs verification against current Microsoft docs).
- **Power Automate** — premium / per-flow licensing applies if the production flows use premium connectors or run as standalone (non-app-triggered) flows. (Needs verification against current Microsoft docs.)
- **Environment / DLP** — production will need a properly governed environment and DLP policy; the prototype's sandbox almost certainly does not match it. Use [estimate-licensing](../docs/licensing-and-capacity.md) thinking and the CLI `estimate-licensing` command to produce a directional cost sheet.
