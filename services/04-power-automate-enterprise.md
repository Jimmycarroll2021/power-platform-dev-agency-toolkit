...
- **Premium connectors.** Any flow that uses a premium connector (e.g. HTTP, custom connectors, SQL Server, Azure, most third-party LOB connectors) requires a standalone premium plan (Power Automate Premium per-user, or a Process licence assigned to the flow) for the licensed entity. Standard Microsoft 365 seeded use rights cover standard connectors only — they do not cover premium connectors (the narrow exception is a seeded/inherited licence used strictly *in-context* with the parent app). ([Microsoft Learn — Types of Power Automate licenses](https://learn.microsoft.com/en-us/power-platform/admin/power-automate-licensing/types), [Power Automate licensing FAQ](https://learn.microsoft.com/en-us/power-platform/admin/power-automate-licensing/faqs))
- **Per-user vs per-flow plans.** Power Automate supports both user licensing (Power Automate Premium per user) and capacity licensing assigned to the automation. The former per-flow plan has been folded into the **Process licence**, which licenses a specific business process/flow for unlimited users regardless of who runs it — economical for shared, high-throughput processes. Process licences are environment-specific, and up to 10 can be stacked on one cloud flow (each adds 250,000 actions/day). Choose deliberately based on user count vs flow/process count. ([Microsoft Learn — Types of Power Automate licenses](https://learn.microsoft.com/en-us/power-platform/admin/power-automate-licensing/types), [Deep dive on specific licenses](https://learn.microsoft.com/en-us/power-platform/admin/power-automate-licensing/deep-dive-on-specific-license))
- **API request limits / throttling.** Power Platform enforces daily Power Platform Request (PPR) limits tracked per user (not pooled across users) and separate per-connector service-protection throttling. The two are distinct: a flow can be throttled by a connector (e.g. SharePoint's per-minute action cap) without hitting its daily PPR allowance, and vice versa. High-volume flows can exhaust limits or be throttled; design for batching, concurrency control, and back-off. Exact request allowances are licence- and time-dependent — check the current entitlement table. ([Microsoft Learn — Requests limits and allocations](https://learn.microsoft.com/en-us/power-platform/admin/api-request-limits-allocations), [Limits of automated, scheduled, and instant flows](https://learn.microsoft.com/en-us/power-automate/limits-and-config))
- **Custom connectors.** Using a custom connector is a premium operation — the flow/user must carry premium entitlement (Power Automate Premium or a Process licence on the flow) even though you authored the connector. Seeded/inherited licences only cover custom connectors used in-context with the parent app. ([Microsoft Learn — Power Automate licensing FAQ](https://learn.microsoft.com/en-us/power-platform/admin/power-automate-licensing/faqs))
- **AI Builder / Copilot credits.** If a flow invokes AI Builder (e.g. document processing, prediction), it consumes capacity credits — a separate purchase/allocation. As of 1 November 2025, AI Builder features in Power Apps and Power Automate cloud flows consume **AI Builder credits first, then fall back to Copilot Credits** when AI Builder credits are absent or exhausted; document processing is metered per page and capacity resets monthly with no rollover. AI Builder capacity add-ons are now purchasable only as a renewal/true-up by existing customers, and seeded AI Builder credits are scheduled for removal on 1 November 2026. Estimate volume × per-capability credit cost before committing. ([Microsoft Learn — AI Builder licensing overview](https://learn.microsoft.com/en-us/ai-builder/administer-licensing), [Licensing and Copilot Credits](https://learn.microsoft.com/en-us/ai-builder/message-management))
- **Copilot Studio capacity.** If automation triggers or is triggered by a Copilot Studio agent, that consumes Copilot Studio capacity — a distinct meter. Note the terminology change: as of 1 September 2025 the agent currency moved from "messages" to **Copilot Credits** (same pack quantities and pay-as-you-go rate). Capacity is pooled at the tenant, allocated to environments, resets monthly with no rollover, and overage can be handled via reallocation, more capacity, or a pay-as-you-go meter. ([Microsoft Learn — Manage Copilot Studio credits and capacity](https://learn.microsoft.com/en-us/power-platform/admin/manage-copilot-studio-messages-capacity), [Copilot Studio licensing](https://learn.microsoft.com/en-us/microsoft-copilot-studio/billing-licensing))
- **Dataverse capacity.** Flows that read/write Dataverse contribute to **database, file, and log** capacity consumption, which are metered and reported separately. System-generated logs (audit logs, plug-in trace logs) count against the log entitlement, so high-volume logging — or large attachments against file storage — can move the capacity needle. The default environment ships with 3 GB database, 3 GB file, and 1 GB log capacity; overages can be handled via pay-as-you-go, reallocation, or increased entitlement. ([Microsoft Learn — Dataverse capacity-based storage details](https://learn.microsoft.com/en-us/power-platform/admin/capacity-storage))
