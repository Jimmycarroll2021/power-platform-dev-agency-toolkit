...
- "Our SharePoint list hit the 5,000-item view threshold / delegation limits."
...
- **Premium licensing required.** Custom Dataverse tables and apps built directly on Dataverse are **premium** — every interactive user generally needs a qualifying license (Power Apps Premium per-user, Power Apps per-app, or an entitling Dynamics 365 license). Seeded/limited-use Dataverse entitlements bundled with some Microsoft 365 / Dynamics plans do **not** cover full custom-table use. (Needs verification against current Microsoft docs.)
- **Dataverse database capacity.** Tables consume **Database** capacity; logs/audit data consume **Log** capacity; files/images consume **File** capacity. Each is metered and billable beyond the tenant default allowance. High auditing and large file columns can consume capacity fast. (Needs verification against current Microsoft docs.)
- **API request limits / service protection.** Plug-ins, workflows, and integrations consume API entitlements and are subject to service-protection limits; heavy synchronous plug-ins can throttle. Size request volume against per-license daily API allowances. (Needs verification against current Microsoft docs.)
- **Premium connectors.** Any flow/plug-in pattern that calls premium connectors (HTTP, custom connectors, SQL, etc.) requires premium licensing on the relevant principal/user. (Needs verification against current Microsoft docs.)
- **AI Builder credits.** If the model feeds AI Builder (e.g., prediction on Dataverse rows), that consumes **AI Builder credits** purchased separately — not included in Power Apps licensing. (Needs verification against current Microsoft docs.)
- **Copilot Studio messages.** If a Copilot Studio agent reads/writes this Dataverse model, it consumes **Copilot Studio message** capacity, billed separately. (Needs verification against current Microsoft docs.)
- **Environment strategy affects cost.** Separate Dev/Test/Prod environments each carry capacity implications; sandbox vs production environment types differ. Plan environment topology before building (see governance/ALM services).
...
- **License gaps at go-live** — users blocked because premium licensing was assumed, not confirmed. *Mitigation:* verify license coverage against current Microsoft docs and the licensing checklist before deployment. (Needs verification against current Microsoft docs.)
