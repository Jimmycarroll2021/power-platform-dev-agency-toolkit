...
> The per-call credit figures above are **illustrative order-of-magnitude estimates only**. Microsoft does not publish a single public per-model credit table on Learn; the authoritative numbers live in the [AI Builder capability rate table](https://learn.microsoft.com/en-us/ai-builder/administer-licensing) and the [Microsoft Power Platform Licensing Guide (pdf)](https://go.microsoft.com/fwlink/?LinkId=2085130). One verified worked example: **receipt processing = 32 AI Builder credits per receipt** ([credit-management](https://learn.microsoft.com/en-us/ai-builder/credit-management)) — note this is well below the "~50-100" estimate above, so treat the table as directional, not authoritative. Capabilities are grouped into Basic/Standard/Premium/Content-Processing tiers that drive the rate. Always price from the rate card before estimating costs.
