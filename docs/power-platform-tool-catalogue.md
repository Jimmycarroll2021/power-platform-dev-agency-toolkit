...
| **Key limitations and quotas** | 100MB message size limit (use chunking for larger), 28-day default run-history retention, 30-day flow timeout. Action throughput is governed by Power Platform request limits / action limits (100,000 actions per 5-minute burst, daily entitlements by license) rather than a fixed "100k actions per run" cap — see [limits and config](https://learn.microsoft.com/en-us/power-automate/limits-and-config) and [Power Platform request limits](https://learn.microsoft.com/en-us/power-platform/admin/api-request-limits-allocations) |
